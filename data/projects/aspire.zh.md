...
.NET Aspire 是一个代码优先、可扩展的工具链，用于构建可观测、生产就绪的云原生应用。它通过统一的应用模型将服务、资源与连接声明为单一的来源，让开发者能够用一条命令在本地启动、调试整个分布式应用，并使用相同的组合将应用部署到 Kubernetes、云端或自有服务器上。

## 主要特性

Aspire 提供代码优先的应用模型与项目模板以减少手工配置，一体化 CLI 用于本地启动、调试及生成部署清单，可视化仪表盘与服务发现提升可观测性与调试效率，以及与 Kubernetes 与常见云平台的无缝集成。丰富的示例与模板便于快速上手与企业化扩展。
